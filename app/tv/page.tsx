...
function buildSlotWindow(flowers: Flower[], hiIdx: number): { vis: Flower[]; hiW: number; hi: Flower | undefined } {
  ...
  //    Each "cycle" = hiIdx advances → we shift which batch of SALE/SAT/IND is visible
  ...
  // Overflow sale items go back to SAT/IND by type (same as TOP/MUST extras)
  ...
  // IND: fills remaining slots (at least CAP_IND, up to whatever's left of MAX_VIS)
...
export default function TVMenuPage() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setHighlights(prev => {
        const next = {...prev};
        // hiIdx cycles through totalCount * MAX_VIS so batch rotation advances
        // buildSlotWindow uses floor(hiIdx / MAX_VIS) for batch cycling
        for (const t of TIERS) {
          const total = flowers[t]?.length || 1;
          const range = Math.max(MAX_VIS, total * MAX_VIS);
          next[t] = ((prev[t]||0)+1) % range;
        }
