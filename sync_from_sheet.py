"""
{{STORE_NAME}} - Auto Sync from Google Sheets
==========================================
...
a filtered menu for {{STORE_NAME}} (ALC01).
...
# ============================================================
STORE_CODE = "{{STORE_CODE}}"  # After Dark store code
...
def main():
    print("=" * 60)
    print(f"  {{STORE_NAME}} - Product Sync (Store: {STORE_CODE})")
    print("=" * 60)
    print()
    ...
    if do_deploy:
        ...
        if result.returncode == 0:
            print("  [OK] Live at https://{{DOMAIN_NAME}}")
        else:
            print(f"  Output:\n{result.stdout[-500:]}")
    else:
